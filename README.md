## Postman Collection

This Postman collection includes:

- User signup (with random user generation)
- User signin with JWT authentication
- CRUD operations for movies (GET, POST, PUT, DELETE)
- Error handling tests (duplicate user, missing required fields)

### Postman Collection
[Download / View Collection](https://github.com/itsh1ghn00n/CSC3916_Assignment3/blob/main/postman_collection.json)

### Postman Web View
https://itsh1ghn00n-2653726.postman.co/workspace/Nico_C_CSCI3916~bfc22366-62b0-43f1-b51d-db6bdb7b407d/collection/51837659-3f977d44-76b2-4e57-b1ea-d0c040ff49eb?action=share&creator=51837659&active-environment=51837659-18ad8c38-f62a-462b-86b6-56a08ebb71de


### React App
Render Site: (https://csc3916-react-6fl6.onrender.com)

### Live API
API Site: (https://csc3916-assignment3-eoiw.onrender.com)